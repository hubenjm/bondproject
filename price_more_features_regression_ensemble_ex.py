...
def main():
	...
	d = d.drop(['state', 'name', 'issuetype', 'bidcount', 'issuesource', 'tradetype', 'cusip', 'tradeid', 'tradedate'], axis = 1)
	d = pd.concat([d, d_state, d_other, d_name_features], axis = 1)	

	column_names = d.columns
	d = d.dropna() #gets rid of several rows where issue size is null
	price = d.pop('price')
	...
	#"Gradient Boosted Tree", "Logistic Regression"]
